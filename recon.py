...
class AcunetixAPI:
    ...
    def test_connection(self):
        """Test API connection."""
        try:
            logger.info("🔄 Đang kiểm tra kết nối tới Acunetix API...")
            ...
            if response.status_code == 200:
                logger.info("✅ Kết nối thành công tới Acunetix API")
                return True
            else:
                logger.error(
                    f"❌ Lỗi kết nối Acunetix API: {response.status_code} - {response.text}")
                return False
        except requests.RequestException as e:
            logger.error(f"❌ Lỗi khi kết nối tới Acunetix API: {str(e)}")
            return False
    ...
    def add_target(self, target_url, description):
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                logger.error(
                    f"❌ Failed to add target: {response.status_code} - {response.text}")
                return None
    ...
    def start_scan(self, target_id, scan_profile="Full Scan"):
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                logger.info(
                    f"✅ Started scan for target ID {target_id} (Scan ID: {scan_id})")
                return scan_id
            else:
                logger.error(
                    f"❌ Failed to start scan: {response.status_code} - {response.text}")
                return None
        except Exception as e:
    ...
    def _get_scan_profile_id(self, profile_name):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"❌ Failed to get scanning profiles: {response.status_code} - {response.text}")
                return None
        except Exception as e:
...
class ReconTool:
    def __init__(self, domain, output_dir=None, max_workers=5, acunetix_config=None):
        ...
        if self.acunetix_config:
            ...
            if not self.acunetix_api.test_connection():
                logger.error(
                    "❌ Không thể kết nối tới Acunetix API. Dừng khởi tạo ReconTool.")
                sys.exit(1)
        ...
        # Define output files
        self.subfinder_httpx_file = os.path.join(
            self.output_dir, "output-subfinder-httpx.txt")
    ...
    def check_tools(self):
        ...
        for tool in required_tools:
            try:
                subprocess.run([tool, "-version"], stdout=subprocess.PIPE,
        ...
        if missing_tools:
            logger.error(
                f"Please install the following tools: {', '.join(missing_tools)}")
            return False
        return True
    ...
    def prepare_acunetix(self):
        ...
        if os.path.exists(self.urls_file):
            logger.info(
                f"✅ URL list ready for Acunetix scan at: {self.urls_file}")
            ...
            if self.acunetix_api and self.acunetix_api.test_connection():
                ...
                target_ids = self.acunetix_api.add_targets_from_file(
                    self.urls_file, description_prefix)
                if not target_ids:
                    logger.warning("No targets were added to Acunetix")
                    return True

                # Start scans for each target
                scan_profile = self.acunetix_config.get(
                    'scan_profile', 'Full Scan')
                scan_ids = []

                for target_id in target_ids:
                    scan_id = self.acunetix_api.start_scan(
                        target_id, scan_profile)
                    if scan_id:
                ...
                logger.info(
                    f"✅ Acunetix scan information saved to {os.path.join(self.output_dir, 'acunetix_scans.json')}")
            else:
                logger.info(
                    "To use with Acunetix, import this file into your Acunetix scanner or configure the API.")
    ...
    def full_recon(self, nuclei_templates_path="~/nuclei-templates"):
        ...
        for step_func, args, description in steps:
            ...
            if not result:
                logger.error(
                    f"❌ {description} failed. Check the logs for details.")
                # Continue with next step instead of stopping

        logger.info(f"\n{'='*20} RECONNAISSANCE COMPLETED {'='*20}")
        logger.info(
            f"All results are saved in: {os.path.abspath(self.output_dir)}")
        return True
...
def main():
    ...
    parser.add_argument(
        "domain", help="Target domain (e.g., example.com or *.example.com)")
